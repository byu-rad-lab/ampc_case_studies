...
parser.add_argument('-D', '--dir', type=str, default='/tmp/ampc24/arm/lin_vs_aff',
...
if reference_type == 'step':
    ...
    traj_fn = traj.Ramp(velocity=amplitude, offset=x0[0], horizon=T, dt=dt)
else:
    traj_fn = traj.Sinusoidal(amplitude, frequency_hz, T, dt, reference_type)

# traj_fn = traj.Ramp(velocity=2*np.pi/5, offset=0.0, horizon=T, dt=dt)
...
if type(c_list) == float:
    ...
    # cost, x_hist, xr_hist, u_hist, st_hist = sim.run(x0, c=1.0, linearize=True)
    # lin_cost_list.append(cost)
    # lin_xtraj_hist.append(x_hist)
    # lin_utraj_hist.append(u_hist)
    # lin_st_trials.append(st_hist)
    ...
    for i in range(len(c_list)):
        # plotter.plotStateLine(aff_xtraj_hist[i], label=f'c = {c_list[i]}')
        plotter.plotStateLine(aff_xtraj_hist[i], label=f'aff: c = {c_list[i]}')
        plotter.plotStateLine(lin_xtraj_hist[i], label=f'lin: c = {c_list[i]}')
    # plotter.plotStateLine(x_hist, label='lin')
    plotter.plotStateLine(xr_hist, 'r--', label='ref')

    if save:
        np.savetxt(args.dir + 'time.txt', sim.time)
        np.savetxt(args.dir + 'costs.txt', np.array(aff_cost_list))
        np.savetxt(args.dir + 'states.txt', np.array(aff_xtraj_hist).reshape(len(c_list), -1))
        np.savetxt(args.dir + 'ref_states.txt', xr_hist)
